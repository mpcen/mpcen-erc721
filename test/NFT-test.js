...
describe('NFT', () => {
    ...
    describe('Contract initialization', () => {
        ...
        it('Has correct max mint amount', async () => {
            expect(await contract.MAX_MINT_AMOUNT()).to.equal(20);
        });
        ...
        it('Has correct baseURI', async () => {
            expect(await contract.getBaseURI()).to.equal('ipfs://initial_base_uri/');
        });
    ...
    describe('Paused', () => {
        ...
        it('Prevents presale minting when contract is paused', async () => {
            await expect(contract.connect(otherAccount).mintPresale(1)).to.be.revertedWith(
                'Sale is not presale'
            );
        });
    ...
    describe('Presale', () => {
        ...
        it('Lets the owner set the allow list', async () => {
            ...
            expect(await contract.getAllowedMintCount(otherAccount.address)).to.equal(3);
            expect(await contract.getAllowedMintCount(otherAccount2.address)).to.equal(3);
            expect(await contract.getAllowedMintCount(otherAccount3.address)).to.equal(0);
        });

        it('Prevents non-whitelisted users from minting during presale', async () => {
            const allowList = [otherAccount.address, otherAccount2.address];
            await contract.setAllowList(allowList, 2);
            await expect(contract.connect(otherAccount3).mintPresale(1)).to.be.reverted;
            await expect(contract.connect(otherAccount3).mint(1)).to.be.revertedWith(
                'Sale is not public'
            );
        });
        ...
        it('Lets whitelisted users mint during presale state', async () => {
            ...
            expect(await contract.connect(otherAccount2).mintPresale(1))
                .to.emit(contract, 'Transfer')
                .withArgs(ethers.constants.AddressZero, otherAccount2.address, 3);
        });
    ...
    describe('Public Minting - Unrevealed', () => {
        ...
        it('Non-owners can successfully mint multiple NFTs for a cost', async () => {
            ...
            expect(
                ...
                .withArgs(ethers.constants.AddressZero, otherAccount.address, 1)
                .withArgs(ethers.constants.AddressZero, otherAccount.address, 2);
            expect(await contract.tokenURI(1)).to.equal('ipfs://initial_base_uri/');
            expect(await contract.tokenURI(2)).to.equal('ipfs://initial_base_uri/');
        });
    });
    ...
    describe('Public Minting - Revealed', () => {
        ...
        it('Rejects non-owners from minting without enough funds', async () => {
            await expect(
                contract.connect(otherAccount).mint(1, { value: web3.utils.toWei('0.01', 'ether') })
            ).to.be.revertedWith('Insufficient funds');

            await expect(
                contract.connect(otherAccount).mint(1, { value: web3.utils.toWei('0.05', 'ether') })
            ).to.emit(contract, 'Transfer');
        });
        ...
        it('Prevents users from minting more than the maxMintAmount', async () => {
            ...
            await expect(
                contract.connect(otherAccount).mint(21, { value: totalCost.toString() })
            ).to.be.revertedWith('Exceeds allowed mint amount');

            totalCost = web3.utils.toBN(costPerMint * 20);

            expect(
                await contract.connect(otherAccount).mint(20, { value: totalCost.toString() })
            ).to.emit(contract, 'Transfer');
        });

        it('Prevents users from minting more than the maxSupply', async () => {
            const costPerMint = await contract.COST();
            let totalCost = web3.utils.toBN(costPerMint * 20);

            expect(await contract.mint(20, { value: totalCost.toString() })).to.emit(
                contract,
                'Transfer'
            );
            expect(await contract.mint(20, { value: totalCost.toString() })).to.emit(
            ...
            expect(await contract.mint(19, { value: totalCost.toString() })).to.emit(
                contract,
                'Transfer'
            );

            await expect(
                contract.connect(otherAccount).mint(2, { value: totalCost.toString() })
            ).to.be.revertedWith('Insufficient supply');

            await expect(contract.connect(otherAccount).mint(1, { value: costPerMint })).to.emit(
                contract,
                'Transfer'
            );
        });
    ...
    describe('Non-minting Public functions', () => {
        ...
        it('Returns the correct tokenIds of addresses', async () => {
            ...
            await contract.mint(2, { value: totalCost.toString() });
            ...
            await contract.connect(otherAccount).mint(1, { value: costPerMint });
            await contract.connect(otherAccount).mint(2, { value: totalCost.toString() });
    ...
    describe('Only owner functions', () => {
        ...
        xit('Only the owner can set different costs', async () => {
            ...
            await expect(
                contract.connect(otherAccount).setCost(web3.utils.toWei('1', 'ether'))
            ).to.revertedWith('Ownable: caller is not the owner');
        });

        it('Only the owner can set max mint amount', async () => {
            let maxMintAmount = await contract.MAX_MINT_AMOUNT();

            expect(maxMintAmount).to.equal(20);
            await contract.setMaxMintAmount(3);
            maxMintAmount = await contract.MAX_MINT_AMOUNT();
            expect(maxMintAmount).to.equal(3);

            await expect(contract.connect(otherAccount).setMaxMintAmount(2)).to.revertedWith(
        ...
        it('Only the owner can set the not revealed uri', async () => {
            ...
            await expect(contract.connect(otherAccount).setBaseURI('something')).to.revertedWith(
                'Ownable: caller is not the owner'
            );
        });
        ...
        it('It reserves a set amount of NFTs', async () => {
            ...
            expect(await contract.balanceOf(owner.address)).to.equal(reservedAmount);

            for (let i = 0; i < reservedAmount; i++) {
                expect(await contract.tokenOfOwnerByIndex(owner.address, i)).to.equal(i + 1);
            }
        });
        ...
        it('Only the owner can withdraw funds into their wallet', async () => {
            ...
            expect(Number(web3.utils.fromWei(originalBalance.toString(), 'ether'))).to.be.closeTo(
                9994.7,
                0.01
            );
            ...
            expect(Number(web3.utils.fromWei(newOwnerBalance.toString(), 'ether'))).to.be.closeTo(
                9995.7,
                0.01
            );
